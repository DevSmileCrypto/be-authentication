...
import io.cryptobrewmaster.ms.be.authentication.exception.integration.RemoteMsPassThroughException;
...
public class GlobalControllerExceptionHandler {
    ...
    public ResponseEntity<ErrorInfoDto> handleServletException(ServletException e, HttpServletRequest request) {
        ErrorInfoDto errorInfoDto = createErrorDto(
                ExceptionCode.SOME_PARAMETERS_ABSENT_OR_INVALID_EXCEPTION,
    ...
    public ResponseEntity<ErrorInfoDto> handleRuntimeException(RuntimeException e, HttpServletRequest request) {
        ErrorInfoDto errorInfoDto = createErrorDto(
                ...
                ExceptionCode.UNKNOWN_EXCEPTION.getDebugMessage()
        );
        logError(request, e);
    ...
    public static ErrorInfoDto createErrorDto(BaseException baseException) {
    ...
    public static ErrorInfoDto createErrorDto(ExceptionCode exceptionCode, String message) {
        ErrorInfoDto errorInfoDto = new ErrorInfoDto();
        errorInfoDto.setCode(exceptionCode.getCode());
        errorInfoDto.setStatus(exceptionCode.getHttpStatus().value());
    ...
    private void log(HttpServletRequest request, BaseException baseException) {
        String message = request.getRequestURI() + "?" + request.getQueryString() + MESSAGE_PREFIX + baseException;
